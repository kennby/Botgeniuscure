  ...
  setTimeout(() => {
    ...
    const code = lastDisconnect?.error?.output?.statusCode || lastDisconnect?.error?.output?.payload?.statusCode;
     ...
     if (code !== DisconnectReason.connectionClosed){ 
        parent.sendMessage(conn.user.jid, {text : ``}, { quoted: m }) //reconectar
    } else {
        parent.sendMessage(m.chat, {text : ``}, { quoted: m }) // session cerrada
    }
    }
    //----
    ...
    await parent.sendMessage(m.chat, {text : args[0] ? `` : ``}, { quoted: m })
    await sleep(5000)
    if (args[0]) return
		await parent.sendMessage(conn.user.jid, {text : ``}, { quoted: m })
...
handler.help = ['serbot']
handler.tags = ['bebot']
handler.command = ['serbot --code', 'serbot', 'jadibot', 'code', 'botclone']
